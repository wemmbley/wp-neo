...
use Exception;
use Neo\Framework\Helpers\FileSystem\File;
use Neo\Framework\Helpers\FileSystem\Path;
use Neo\Framework\Helpers\Primitives\Arr;
use Neo\Framework\Helpers\Primitives\Str;
use Neo\Framework\Helpers\Regex;
...
class View
{
	...
	{
		static::$templatePath = get_theme_file_path(sprintf('templates/%s.blade.php', $template));

		if ( ! File::exists(static::$templatePath))
			throw new Exception('Template ' . static::$templatePath . ' not found');
	...
	protected static function compileWPLoops(string $template): array|string
	{
...
//		 }
		...
		return
			Regex::replaceAll('/@loop\("(.+)"\)/')
			     ->string($template)
			     ->to('<?php echo $1; ?>');
	}

	protected static function compileShortcuts(string $template): array|string
	{
		$result = Regex::replaceAll('/@header/')
		     ->string($template)
		     ->to('<?php wp_head(); ?>');

		$result = Regex::replaceAll('/@footer/')
		               ->string($result)
		               ->to('<?php wp_footer(); ?>');
		...
		array_walk_recursive($facadeFiles, function($folder, $name) use($facades, &$template) {
			...
			foreach ($facades[2] as $facadeName) {
				if ($facadeName === $nameWithoutExtension) {
					...
					$facadeNamespace = '<?php echo \Neo\\' . $facadeNamespace . '; ?>';
	 ...
	 * @throws Exception
	 */
	protected static function compileIncludes(string $template): array|string
	{
		return
			Regex::replaceAll('/@include\("(.+)"\)/')
			     ->string($template)
			     ->to('<?php Neo\Framework\View::template("$1")->display() ?>');
	}
